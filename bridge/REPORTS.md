...
## [P-064] KB-233 스캔 결과 파파고식 개편 (2026-07-23)

**커밋**: `39eb672` (main, 푸시·CI) · **검증**: tsc 0 · jest 250/250 (+4) · **preview OTA 발행**(양 채널 도달)

- **① 스캔라인 왕복**: withRepeat reverse=true — 위↔아래 핑퐁, 리셋 점프 소멸
- **② 파파고식 결과 화면**: 다크 시트 컨테이너·캡션·범례 칩 삭제(어제 만든 `scan.resultCaption` ×10 정리 포함) → **사진 풀블리드 + (사진 뷰) 하단 옅은 그라데이션 + 원형 버튼 4개만 플로팅**(활성 주황·safe-area). degradedNote는 안전 고지라 버튼 위 유지(삭제 목록에 없어 존치 판단)
- **③ 원본 피크**: 위험도 뷰 빈 영역 꾹 → 마커·버튼 150ms 페이드아웃(버튼은 pointerEvents 차단 겸), 뗌 즉시 복귀. 마커가 위 레이어라 마커 탭과 비간섭
- **④ 핀치 줌+팬+더블탭**: RNGH Pinch/Pan/Tap(Race+Simultaneous) — **이미지+마커 동일 transform 컨테이너**(확대해도 마커 정위치 추종). `zoom.ts` 순수 클램프(1~4x·경계 팬=확대분 절반·더블탭 2.5x 토글/리셋). 1x에선 팬 무시(피크 롱프레스와 비간섭), 줌 상태 마커 탭 동작. 원본 뷰도 동일(같은 컴포넌트)
- 테스트 +4: 클램프 유닛 3케이스 · 피크 토글(꾹=pointerEvents none/뗌=복귀) · 파파고 레이아웃(캡션·범례 부재+버튼 4) 갱신 · scan 테스트 4본 RNGH mock 보강
...
- **⓪ [반려 보수] 셔터 레이스**: 원인 분석 그대로 — state 가드는 리렌더 전 탭을 못 막음. `capturingRef` **동기 가드**(진입 즉시 검사·세트)로 실차단, state는 시각 disable 전용 병행. capture·pickFromGallery 동일. **레이스 테스트 잠금**: 픽커 pending 중 재탭 → 호출 1회
- **① 샘플 스캔 제거**: SAMPLE_DISHES·카메라/에러 화면 버튼·`scan.sample` ×10 삭제. 권한 거부 화면엔 갤러리 버튼 유지. 기존 테스트 2본은 갤러리 경로(OCR mock → 실 segmentMenu)로 결과 진입 재배선
- **② D2 스캐닝 오버레이**: 사진 배경 + 흰 코너 브래킷 4 + **주황 스캔라인 위→아래 반복 스윕**(글로우 그라데이션 트레일, 1.8s 루프 — reduced-motion 시 전역 config 정지) + `scan.reading` 재사용 캡션+소형 스피너 + 좌상단 X 유지. 갤러리 경로 동일
- **③ D3 하단 바**: 다크 시트(상단 라운드+드래그 핸들) — `scan.resultCaption`(신규 ×10) + 위험도 범례 칩 3(RiskMark SVG + 기존 risk.* 라벨 — 색+아이콘 병행, 색맹 축) + **원형 버튼 4**(리스트=IconList 신규/위험도=IconScanLines/원본=IconGallery/다시찍기=IconRetry — 라벨 기존 키 재사용, **활성=주황 원**, 사진저장 제외 확정 반영). 기존 세그먼트(P-032 글라이드)·다시찍기 Btn 대체
- 테스트 +3: 레이스 · 샘플 부재 · D3 4버튼/활성 주황
