...
## Q-20 ⬜ [오늘의 메인] QA 빌드(iOS 빌드8·안드 빌드3, dev 임베드 — P-069)에서 확인

> **[커맨드 센터 7/24] 플랜**: 오늘 = QA 빌드(env=dev, 네이티브 전부 탑승·재베이스라인) → 내일 = 종한 prod 신호 → P-067 → 제출 빌드(빌드9·빌드4 AAB, env=prod) → 스토어 업로드. BE 주소는 JS env라 오늘의 네이티브 QA가 내일 빌드에 그대로 유효.

- [ ] 안드 스플래시 로고 원형 잘림 해소 — 로고-only 에셋 (P-054)
- [ ] 설정>앱>K-Bap>언어에 10종 노출(iOS·안드13+) + OS 언어 변경 시 앱 재시작 반영 (P-060)
- [ ] 스캔 결과 메뉴명 = 앱 언어로 (BE /scans lang 반영 후 — P-060 ②③)
- [ ] A90(안드12): 폰 전체 언어 추종·프로필 언어 행 숨김 정상
- [ ] 스캔 결과 화면: 하단 그라데이션(사진이 하단 채울 때)·리스트 끝 카드 완전 가시·버튼 정위치·피크 시 그림자 동반 소멸 (P-066·P-068 — Metro로 먼저 봐도 됨)
- [ ] **원거리·조밀 메뉴판 마커 정합** — 노트북 화면을 멀리서 찍어도 마커 사다리 없음 (P-070 OTA 후) + 근접 촬영 무회귀
- [ ] 스캔 완료 직후 **사진(위험도) 뷰가 기본**으로 뜸 (P-071 OTA 후)
- [ ] 안드 하단 내비바 6곳(3버튼 내비) — Q-14 잔여, 빌드3에서 같이 (P-055 → KB-225)
- [ ] (아이폰 빌드8) Q-05 재확인: 로그인→삭제→재설치→게스트 진입 시 홈 포함 잔상 0 (→ KB-152)

## Q-14 🔄 Android 공기계 첫 스모크 (KB-176 완료됨 — 잔여: 내비바 → KB-225)

> **현재 상태(7/22)**: 설치·게스트 렌더·구글 로그인·온보딩·언어 지역화·언어 선택 회색(P-024) 전부 ✅. **KB-176은 7/23 완료 전환됨**(마커 포함 스모크 종결). 이 섹션 잔여 = 안드 하단 내비바 6곳(P-055 → KB-225): ① 게이트 시트 '나중에 하기' ② 로그인 하단 ③ 인트로 CTA ④ 스캔 하단 바 ⑤ owner/order Done ⑥ 온보딩 CTA(회귀).
>
...
### ✅ Q-14 종결 체크리스트 (7/24 — 이것만 하면 끝. 안드 공기계 **빌드3** + 3버튼 내비 켜고)

- [ ] ① 게이트 시트(게스트로 스캔 탭) — '나중에 하기' 안 짤림
- [ ] ② 로그인 화면 — 하단 버튼 안 짤림
- [ ] ③ 인트로 — CTA 버튼 안 짤림
- [ ] ④ 스캔 카메라 — 하단 바(셔터·갤러리) 안 짤림
- [ ] ⑤ 사장님/주문 카드 — Done 버튼 안 짤림
- [ ] ⑥ 온보딩 — CTA 안 짤림 (회귀 확인)
- [ ] (덤) 애플 로그인 버튼: 안드에서 미노출 or 무동작 = 정상 (노출+탭 크래시만 ⚠️)

전부 ✅면 "Q-14 종결해줘" → 커맨드 센터가 Q-14 종결 + KB-225 완료 전환.
나머지 번호(설치·게스트·로그인·스캔 플로우·가로 회전·갤러리 피커·마커)는 근거 축적으로 확인 완료 처리됨.
...
## Q-19 ✅ 7/23 OTA 재확인 묶음 (P-057·P-058·P-061 → KB-212 후속·KB-125 후속·KB-231)

[환경: 테플·공기계 공통 — 강제종료→2회 실행 후. ⚠️ 이 OTA부터 인앱 언어 피커가 사라짐(P-060 정상 동작) — 언어 테스트는 폰 전체 언어 변경으로]

- [x] 스캔 셔터 연타 ✅ **종결(7/23)** — P-062 ref 가드 후 A90 USB adb 실측: 연타 재현 중 `[scan] photo =` **1회**(가드 정상 작동, 예진 재확인 "한번만 되는게 맞아"). 1차 보수(P-061 state 가드)는 레이스에 뚫렸었고 P-062 동기 ref가 최종 해결
- [x] (P-062 OTA 발행됨 — 재실행 2회 후) 셔터 연타 재확인 · 샘플 버튼 없음 · 스캔 로딩=사진+스캔라인 오버레이(D2) · 결과 하단 바=범례+원형 4버튼(활성 주황, 사진저장 없음, D3)
- [x] (P-063 OTA 후) 랭킹 전체 등급 = 리스트 카드 7행(확정 스샷대로) · 나머지 랭킹 화면 무변
- [x] (P-064·P-065 OTA 재발행됨 — 재실행 2회 후) 스캔라인 위↔아래 왕복 · 결과 화면 검정 바탕 없이 버튼만 플로팅 · 꾹=원본 피크 · **핀치 줌 실동작**(워클릿 보수 반영— 에러 0·마커 추종·더블탭)
- [x] (dev Metro — 종한 버킷 수정 확인 후) 스캔 결과 메뉴명이 **앱 언어로**(P-060 완결분·dev 신계약) — .env 주석 해제 후 확인
- [x] 회피 미설정 계정: 스캔·목록에 safe 초록 그대로 (주의 강등 폐지 — 헌법 v2.1.0)
- [x] 프로필 > 안전 고지 행 → 고지 페이지 열림
- [x] 알림 벨·패널 어디에도 없음
- [x] 스캔 배너 = 리스트 첫 카드(밝은 틴트·정상 간격), 위험도/원본 뷰에선 미노출 (P-057)
- [x] 랭킹 다양성(Food variety) 행 dim+"다음 업데이트" (P-058)

> **[커맨드 센터 7/24] 전 항목 확인 완료 — 섹션 종결** (연계 티켓 KB-212·231·232·233 분 반영, 핀치·다양성·미설정 safe 포함)
