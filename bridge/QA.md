...
### 피드백
- **[커맨드 센터 7/20]** 2번·6번은 **같은 원인**: 칩 요약 줄이 0개 땐 높이 0 → 첫 칩에서 높이가 생기며 아래 목록을 밀어냄(1회 점프). 나머지(3·4·5) 통과. **해결 = 요약 줄에 고정 최소 높이 예약**(빈 상태에도 칩 한 줄 높이 유지 + placeholder 텍스트 "선택한 재료가 여기 표시돼요" 류) → 0→1 전환에 레이아웃 높이 불변 → 밀림 0. **KB-178 재수정 P-026 등록**(JS-only, OTA). 확인 후 KB-178 완료 전환.
...
- **[커맨드 센터 7/20]** Q-08 핵심 1~3 ✅. 음식탭 UI 2건:
  ...
  - **1번(지금 인기)**: 조사 결과 **하드코딩 아님** — `search.tsx`가 실제 카탈로그를 `popularityRank`로 정렬한 상위 6개(주석 "인기순 API 미배포→배포 시 교체"). rank가 고정이라 늘 같은 6개=하드코딩처럼 보임. **제품 결정 필요(예진)**: ⓐ MVP 그대로 유지(실제 인기순, 무난) / ⓑ 진짜 인기 API 나올 때까지 '지금 인기' 블록 숨기고 최근 검색만 / ⓒ 라벨만 바꿈. **오프라인 검색**: 현재도 최근 검색(로컬)은 뜸·제출 검색은 에러 분기 있음(search.isError L142). 개선안: 오프라인이면 '지금 인기'(네트워크 필요) 대신 오프라인 안내+최근 검색만 노출. → 방향 정해주면 FE 등록. 지금은 P-027(2번)만 착수.
