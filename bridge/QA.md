...
- [ ] 🆕 **스캔 결과 메뉴명이 앱 언어 무시하고 영어로 뜸 (7/23, A90·S22 공통)** — **FE 무죄 판정(로그 실측)**: 응답이 이미 `"Kimchi Stew"`로 오고 FE는 그대로 표시. `/api/v1/scans` 계약엔 lang 파라미터 자체가 없어 FE가 언어를 보낼 방법이 없음(BE가 DB appLanguage로 결정 — 홈 잔상 P-023과 동일 뿌리). **처방: BE에 lang 추가 요청(타 엔드포인트와 동일 규칙) → 종한 전달 → BE 반영 후 FE 발주**
