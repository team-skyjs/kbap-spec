...
- **[커맨드 센터 7/21]** 원인: P-029가 쓴 아이콘이 손그림 스트로크 근사치. 공식 구글 4색 로고는 `SocialAuthButtons.tsx` 내부에 비공개 함수로 실존(export 안 돼 재사용 실패) / 애플은 로그인=OS 네이티브 버튼이라 SVG 부재 → 공식 필드 마크 신규 추가 필요. **P-034 등록**(구글 export 재사용+애플 공식 마크, OTA). 교체 후 이 항목 재확인.
