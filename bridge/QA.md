...
## 📌 오늘의 QA (7/23 갱신 — 이것만 보면 됨)

**🆕 P-061(KB-231) OTA 발행됨 — 강제종료→2회 실행 후 재확인** (⚠️ P-060 1차분 동반 탑승: 인앱 언어 피커가 사라짐 — 언어 테스트는 폰 전체 언어 변경으로):
- [ ] 셔터 연타해도 촬영 1회만(A90에서 재현했던 것)
- [ ] 회피 미설정 계정: 스캔·목록에 safe 초록 그대로 표시(주의 강등 폐지 — 헌법 v2.1.0)
- [ ] 프로필>안전 고지 행 → 고지 페이지 열림
- [ ] 알림 벨·패널 어디에도 없음
**🆕 P-060 진행 중**: 인앱 언어 피커 제거·OS 언어 추종(즉시분 완료) — /scans lang·appLanguage 철거는 BE 스웨거 대기. OS 설정>앱>K-Bap 언어 노출은 **최종 빌드에서** 확인
**⏳ BE(종한) 대기 2건**: 언어 개편(오늘 중) · 이미지 최적화(PNG 1.5~2MB→webp 리사이즈, A90 로딩 해결 — prod 시딩 전 적기)
...
**✅ 7/22 확인 완료(티켓 종결)**: +사장님 카드 라벨(KB-215)·맵기 원복(KB-195)·게이트 시트(KB-206 — 폴리시1 종결)
**✅ 이전 종결**: 가로 감지(KB-198)·크롭(KB-202)·스캔 효과·스캔 배너(KB-212)·랭킹 정렬/리뷰 정리·press/노드·헤더 바운싱(KB-217)·촬영(KB-218) — kinetics(KB-207)도 종결. **후속 예약**: 랭킹 자잘 UI(예진 상세 대기).
- [ ] 랭킹 다양성(Food variety) 행도 dim+"다음 업데이트" — **P-058 발주됨**(리뷰 적립 기반이라 동일 처리, 예진 7/22 지시). OTA 후 재확인
- [ ] 스캔 배너 A안 재확인 — **P-057 OTA 발행됨**: 배너가 리스트 첫 카드(밝은 틴트·정상 간격), 위험도/원본 뷰에선 미노출
- [x] 위험도(오버레이) 마커 ✅ — **Q-14 종결·KB-176 완료(7/23)**. S22 최초 재현은 OTA 미적용 번들이 원인(같은 APK ≠ 같은 JS) — 강제종료→2회 재실행 후 정위치 확인. 무선 adb 실측 `basis:"reported" 1883×4080`으로 P-056 로직 정상 판정. A90은 카메라가 절반 저장을 안 해 원래부터 비재현 기기.
- [ ] 🆕 **스캔 결과 메뉴명이 앱 언어 무시하고 영어로 뜸 (7/23, A90·S22 공통)** — **FE 무죄 판정(로그 실측)**: 응답이 이미 `"Kimchi Stew"`로 오고 FE는 그대로 표시. `/api/v1/scans` 계약엔 lang 파라미터 자체가 없어 FE가 언어를 보낼 방법이 없음(BE가 DB appLanguage로 결정 — 홈 잔상 P-023과 동일 뿌리). **[7/23 기획 확정 — 언어 대개편으로 확대]**: OS 언어 정본화(인앱 피커 제거·appLanguage 철거·/scans lang) — 정본 `language-settings-brief.md`. BE 전달문 dropbox 작성·**P-060 발주됨**. 최종 빌드에서 재확인
- [ ] 안드 하단 내비바 — **P-055 OTA 발행됨**, 3버튼 내비로: ① 게이트 시트 '나중에 하기' ② 로그인 하단 ③ 인트로 CTA ④ 스캔 하단 바 ⑤ owner/order Done ⑥ 온보딩 CTA(회귀 체크) (→ KB-225)
...
-> 근데 추가로 'food variety'가 리뷰 작성에서 넣기로 한거잖아. 그래서 여기에도 coming in the next update이거 추가해야됨 똑같이 dim하고!


**💻 dev Metro (선택)**: ~~맵기 미조작 body -1 실측~~ → **P-051로 UI 원복 결정(7/22 예진)**: 기본 5 표시·Continue=화면값(5)·Skip=-1. P-051 OTA 후 온보딩 맵기 화면이 예전 모습(불꽃 5개 활성)인지만 확인 ✅ (확인완료, 이 건은 종결, ui 추가작업할수도있음)

- 🔄 안드 스플래시 원형 잘림("K-Bar") — Android 12+ 시스템 마스크가 원인, 안드 전용 로고-only 에셋 분리 (P-054 — 에셋 커밋 완료·재빌드 대기, **안드 빌드3에서 확인**. ⚠️운영: 빌드3 전 OTA엔 2파일 스왑 부활(FE 인지) — 빌드3 재베이스라인으로 소멸)
...
> **현재 상태(7/22)**: 설치·게스트 렌더·구글 로그인·온보딩·언어 지역화·언어 선택 회색(P-024) 전부 ✅. **잔여 = 상단 배너 🤖 목록**(가로 감지·크롭·스캔 효과·배너 등 — 빌드2 탑재분). 완료 시 KB-176 종결.
>
> **[커맨드 센터 7/23 — 종결] 구형 안드(A90·안드12) 갤러리 버튼이 "파일" 화면으로 뜸**: 버그 아님 — 안드 13+의 시스템 포토 피커가 없는 기기는 문서 선택기(DocumentsUI)로 폴백하는 OS 사양. 첫 탭 "최근"은 최근 파일만 보여줌(옛 사진은 ≡→이미지 카테고리에 전부 있음). 이미지 선택→스캔 정상 확인(예진 7/23) → 이슈 없음. 아래 0·3번 진단·구 빌드1 안내는 해결 완료(기록 보존 — 현행 설치는 **빌드2** `24277e9c` 기준).

[환경(현행): 안드 공기계 — **빌드2** 설치: [빌드 페이지](https://expo.dev/accounts/rocher/projects/kbap/builds/24277e9c-2b9e-401b-853d-353c7fc403ea). 구 빌드1은 orphan(OTA 안 감)]
...
[환경: 공기계 **빌드2** — 최신 preview OTA(P-053 제외 전부 반영) 강제종료→2회 실행. 스캔 효과 2종 포함 확인 가능. ⚠️ 6번 스텝 노드=P-042로 "즉시 전환"이 정답으로 변경됨 / 2번 press는 P-042로 헤더 Sign in까지 확대]
...
### 피드백
1. 비로그인상태에서 헤더의 알림 아이콘 눌렀을때 바텀시트 뜨는 애니메이션 & 비로그인시 스캔탭, scan a menu 등 스캔 화면 진입할 때 뜨는 로그인하라는 바텀시트 애니메이션 개이상함, 그냥 원래대로 돌려줘
- **[커맨드 센터 7/22]** P-031 B5의 시트 스프링(SlideInDown.springify)이 원인 — **P-053 등록**(AuthGateSheet 등장만 원복, Snackbar 스프링은 유지). 헤더 P-047과 같은 절제 계열. OTA 후 재확인.
