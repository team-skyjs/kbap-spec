...
- [ ] UI 폴리시 스팟 체크 — Q-18 (P-031·032·035. 스캔 화면 효과 2종은 차기 빌드 제외, 기피 칩=애니 없이 즉시)
- [ ] 랭킹 화면: 현재 카드 기준선 일치 + 점수내역 하단 코너 닫힘 (P-037 — 공기계 preview)
- [ ] 주문카드 스테퍼 −/+ 원 정중앙 (P-040 — 공기계/에뮬 preview, 인스펙터 재확인)
- [ ] Q-18 후속 재확인 (P-042 — 공기계/에뮬 preview): 헤더 Sign in press 반응 · 스테퍼가 Done 바로 위 · 온보딩 스텝 노드 즉시 전환
- [ ] 맵기 미조작 가입 실측 (P-039 — **dev Metro**: 불꽃 안 누르고 계속 → body `spicinessPreference: -1` + 미선택 UI "–/10") → Q-03 종결 겸
- [ ] 빈 프로필 스캔 배너 (P-038 — **dev 또는 빌드2**: build1 preview엔 스캔 화면 미반영)
- [ ] 재설치 홈 잔상 소멸 (P-041 — **프로덕션 OTA 발행됨**: 테플 로그인→삭제→재설치→게스트 진입 시 홈 포함 전 화면 게스트) → Q-05/KB-152 종결

**② 미수행 QA** — Q-03(맵기)·Q-05(Keychain 재설치)·Q-09(탈퇴 잔재)

**🚀 iOS 빌드7 제출됨(7/21 저녁, runtime c43664ed·재베이스라인)** — TestFlight 반영되면 **빌드7 설치 후**: 스플래시 태그라인 없음(KB-194 게이트) · 스캔 배너(P-038) · 스캔 세그먼트 글라이드/스태거(P-032 잔여) · 캡처 크롭 WYSIWYG(P-025 iOS) · Q-05 재확인(P-041)까지 전부 테플에서 확인 가능. dev Metro 항목(P-039 body 실측)만 별도.

**③ 대기**: Q-10(토큰 — 종한 신호) · Q-14 잔여(가로·크롭 — **안드 빌드2 = P-043 발주됨**)
