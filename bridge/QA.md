...
## 📌 오늘의 QA (7/21 기준 — 이것만 보면 됨)

**① OTA 재확인 3건** — 앱 **강제중지→2회 실행** 후 (프로덕션 OTA에도 실려 테플로도 가능):
- [ ] 언어 선택 화면: padding 회색 없음 (P-024 → KB-197 게이트) — **안드 공기계 전용**(iOS는 원래 정상)
- [ ] 온보딩 기피 재료: 첫 선택 시 목록 안 밀림 (P-026 → KB-178 게이트) — 테플/공기계 아무거나
- [ ] 음식탭 기내모드: 헤더 없이 전체화면 오프라인 (P-027 → KB-174 게이트) — 테플/공기계 아무거나
- [ ] 검색화면 기내모드: 최근·지금인기 대신 전체화면 오프라인 (P-028 — **공기계**, preview OTA 발행됨)
- [ ] 프로필>수정 최하단: 가입 수단대로 Apple/Google 표시 (P-029/Q-16 — **공기계**, preview OTA 발행됨)
- [ ] 안전 음식 상세: "이 메뉴 주문하기" → 사장님 카드 (P-030/Q-17 — **공기계**, preview OTA 발행됨)
- [ ] UI 폴리시 스팟 체크 (P-031·032/Q-18 — **공기계**, preview OTA 발행됨)

**② 미수행 QA** — Q-03(맵기)·Q-05(Keychain 재설치)·Q-09(탈퇴 잔재)·Q-15(랭킹 실데이터)·**Q-13(언어 전환 — P-023 프로덕션 OTA 발행됨, 테플에서 지금 가능!** 홈 정상 로드+언어 바꾸면 홈 즉시 새 언어)

**③ 대기**: Q-10(토큰 — 종한 신호) · Q-14 잔여(가로·크롭 — 안드 빌드2 후)
