...
- [ ] (P-062 OTA 발행됨 — 재실행 2회 후) 셔터 연타 재확인 · 샘플 버튼 없음 · 스캔 로딩=사진+스캔라인 오버레이(D2) · 결과 하단 바=범례+원형 4버튼(활성 주황, 사진저장 없음, D3)
- [ ] (P-063 OTA 후) 랭킹 전체 등급 = 리스트 카드 7행(확정 스샷대로) · 나머지 랭킹 화면 무변
- [ ] (P-064·P-065 OTA 재발행됨 — 재실행 2회 후) 스캔라인 위↔아래 왕복 · 결과 화면 검정 바탕 없이 버튼만 플로팅 · 꾹=원본 피크 · **핀치 줌 실동작**(워클릿 보수 반영— 에러 0·마커 추종·더블탭)
- [ ] (dev Metro — 종한 버킷 수정 확인 후) 스캔 결과 메뉴명이 **앱 언어로**(P-060 완결분·dev 신계약) — .env 주석 해제 후 확인
- [ ] 회피 미설정 계정: 스캔·목록에 safe 초록 그대로 (주의 강등 폐지 — 헌법 v2.1.0)
- [✅] 프로필 > 안전 고지 행 → 고지 페이지 열림
- [✅] 알림 벨·패널 어디에도 없음
- [ ] 스캔 배너 = 리스트 첫 카드(밝은 틴트·정상 간격), 위험도/원본 뷰에선 미노출 (P-057)
- [✅] 랭킹 다양성(Food variety) 행 dim+"다음 업데이트" (P-058)

## Q-20 ⬜ [빌드 게이트] 최종 빌드(iOS 8·안드 3)에서 확인

> ⚠️ **[커맨드 센터 7/23] 최종 빌드 전 도메인 결정 필수**: 신계약(scans lang·appLanguage 삭제)이 **dev에만 배포됨** — meogo는 구계약(실측: lang 없음·appLanguage 3곳). 빌드가 meogo 고정인 채 나가면 스캔 언어 개선이 무효. 종한과 정리: ⓐ meogo에도 새 코드 배포 ⓑ prod 시딩+전환 완료 후 eas.json을 prod로. 결정되면 후속 P 발주.
