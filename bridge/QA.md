...
- [ ] 위험도(오버레이) 마커 — **P-056 OTA 발행됨**: 재스캔 시 6개 마커 전부 해당 메뉴 옆 정위치 (→ 통과 시 Q-14 종결·KB-176 완료)
  - [커맨드 센터 7/23] A90 = 6개 정위치 ✅ / S22 = 여전히 하단 밀림(김치·된장만 노출 — 수정 전과 동일 증상). S22 카메라만 파일을 절반 저장해서 이 버그는 원래 S22 전용 — A90은 수정 없이도 정상이라 "A90 잘 뜸"이 수정 적용의 증거가 못 됨. **S22에서 앱 강제종료→2회 재실행(OTA 적용)→재스캔** 필요. 그래도 밀리면 USB 연결 — `[ocr] denominator` 로그로 판정.
