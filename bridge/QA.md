...
- **[커맨드 센터 7/21 — 4번 재확인 반려 2건]** ① 캐시 있는 상태서 기내모드 → 음식탭이 J4 아닌 **캐시 목록+무헤더**(P-027이 ListEmptyComponent 경유라 캐시 있으면 미발동 — 1번 통과는 재시작=캐시 없음이었던 것) ② 그 상태 스크롤 시 푸터 스피너가 **화면을 궤도로 공전**(Spinner 회전 래퍼가 푸터에서 전폭 stretch되는 잠재 버그). → **P-036 등록**(캐시 무관 전체 J4 + 스피너 크기 고정). OTA 후 4·5번 재확인.
