...
- [⚠️] 스캔 셔터 연타해도 촬영 1회만 (A90 재현건) -> 아직 동일함. 수정된거 맞는지 확인해줘
  - [커맨드 센터 7/23] 확인함 — 수정은 들어갔는데 **가드가 비동기 state 기반이라 리렌더 전 연타에 뚫림**(레이스). 검토 때 못 잡은 제 미스. **P-062 0번으로 ref 동기 가드 보수 발주** → P-062 OTA 후 재확인
...
- [ ] (P-064 OTA 후) 스캔라인 위↔아래 왕복 · 결과 화면 검정 바탕 없이 버튼만 플로팅 · 꾹=원본 피크 · 핀치 줌 시 마커 따라옴
