...
- ⚠️ 위험도(오버레이) 뷰: 마커 위치 하향 밀림 + 2/6만 표시 — **P-056 조사·수정 중** (마커 2개는 photoOnly 스펙일 수 있음·위치는 안드 좌표계 버그 유력)
...
- [ ] 게스트 로그인 시트(알림 아이콘·스캔 진입) 등장이 예전처럼 자연스러움 (P-053 — preview OTA 발행됨, 강제종료→2회)
- 🔄 안드 하단 내비바 짤림 전수(시트 '나중에 하기' 등) — P-055 작업 대기, 나오면 하단 고정 UI 쭉 재확인 (P-021 계열 전수판)
- 🔄 안드 스플래시 원형 잘림("K-Bar") — Android 12+ 시스템 마스크가 원인, 안드 전용 로고-only 에셋 분리 (P-054 — 재빌드, **안드 빌드3에서 확인**. KB-194는 iOS 완료 유지)
...
> **현재 상태(7/22)**: 설치·게스트 렌더·구글 로그인·온보딩·언어 지역화·언어 선택 회색(P-024) 전부 ✅. **잔여 = 상단 배너 🤖 목록**(가로 감지·크롭·스캔 효과·배너 등 — 빌드2 탑재분). 완료 시 KB-176 종결. 아래 0·3번 진단·구 빌드1 안내는 해결 완료(기록 보존 — 현행 설치는 **빌드2** `24277e9c` 기준).
