...
> ⚠️ **[커맨드 센터 7/23] 최종 빌드 전 도메인 결정 필수**: 신계약(scans lang·appLanguage 삭제)이 **dev에만 배포됨** — meogo는 구계약(실측: lang 없음·appLanguage 3곳). 빌드가 meogo 고정인 채 나가면 스캔 언어 개선이 무효. **결정(예진 7/23): ⓑ 확정 — meogo 폐기, 내일 prod 전환(P-067 등록됨, 종한 신호 게이트). 오늘 Metro=dev.**
