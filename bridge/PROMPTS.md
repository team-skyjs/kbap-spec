...
## [P-024] ⬜ KB-197 재수정 — 언어 선택 회색은 Android elevation(리플 아님)
...
## [P-023] ⛔ (BE 대기) KB-199 홈 조회 lang 파라미터 — /home?lang=

⛔ **BE 선행 대기**: 종한이 /home에 lang 파라미터를 /foods와 동일하게 추가하고 알려주기로 함(7/20 확정). **알림 오기 전 착수 금지**(그 전엔 400/무시 가능). 알림 오면 이 항목 ⬜로 열림.

원인(Q-13 확정): /home이 요청 언어를 안 받고 회원 DB appLanguage로만 번역 → 언어 전환 잔상 + 미설정/게스트 영어. 캐시·Accept-Language 무관(종한 확인).

### 할 일 (BE 오픈 후)

1. `src/lib/data/useHome.ts` `fetchHome()`: `/home` → `/home?lang=${apiLang()}` (useFoods L77 동일 패턴, 템플릿 리터럴). bootGate 프리페치도 같은 파라미터 정합 확인(키는 이미 ['home', lang])
2. appLanguage PATCH 유지(선호 저장) — 표시는 파라미터가 담당, race 자동 소멸
3. 테스트: 홈 요청 URL에 lang 실리는지 잠금
4. JS-only → preview/production OTA 가능

### DoD

- [ ] 언어 전환 즉시 홈 새 언어(잔상 0)·게스트/미설정 UI 언어 번역(영어 fallback 없음)·tsc 0·jest·OTA
