...
1. `PILL_MAX_W`를 2열 메뉴판에서 열 폭 안에 들어오게 축소(≈화면폭 40%, ~150pt) + displayName 문자 말줄임 동기화 — `estimatePillWidth`와 실렌더 폭 일치 유지
2. 유닛: 긴 영문명 2열 픽스처 → 열 비교차·스태거 0 잠금
3. preview OTA → 예진 같은 메뉴판 재스캔 실증 (전 마커 정위치 예측)

### DoD

- [ ] 긴 이름 2열 비교차 유닛 · 실기 재스캔 밀림 0 · tsc 0 · jest · OTA

완료 시 상태 ✅+커밋 해시, 보고는 REPORTS.md 최상단 [P-072].

## [P-071] ✅ b1c19b9·1eb3a3f KB-233 스캔 직후 기본 뷰 = 사진(위험도) — 신규 유저 멘탈 모델 (예진 확정 7/24)

신규 유저 피드백: "사진을 찍었으니 사진이 보여야지, 리스트가 나와서 황당" — 스캔 완료 시 **기본 뷰를 'risk'(사진+마커)**로 전환. 목록/원본은 하단 버튼으로 기존과 동일 전환.

### 할 일

1. 스캔 완료 진입 뷰 `list` → `risk` (다시 찍기 후 재스캔도 동일)
2. **[추가 7/24] 마커 pill에서 가격 제거 — 번역 메뉴명만** (가격은 리스트·상세 유지). 근거(Metro 실측 계산): 가격 포함 폭 ~220pt가 좌우 열을 실제로 겹치게 해 P-070 실폭 판정이 한 클러스터로 묶음 → 전원 1단(+34pt) 밀림. 제거 시 열 분리 + 줄 간격 36pt>PILL_H 34로 스태거 미발동 = 이 케이스 정위치. estimatePillWidth에서 가격 항 제거 동기화. 마커 스킨 재설계(디자인 시안 대기)와 별개의 선행 보수
3. 기존 `scanDefaultView` 잠금 테스트 반전 + pill 가격 부재 잠금
4. **P-070과 같은 preview OTA로 발행** (새 빌드8/3 runtime 대상) — 발행 후 예진이 같은 메뉴판(노트북 화면) 재스캔으로 정위치 실증

### DoD

- [ ] 스캔 완료 즉시 사진+마커 화면 · 뷰 전환 무회귀 · tsc 0 · jest · OTA(P-070 합류)

완료 시 상태 ✅+커밋 해시, 보고는 REPORTS.md 최상단 [P-071].

## [P-070] ✅ fb41180 KB-240 스캔 마커 사다리 버그 — 겹침 방지(KB-140) 연쇄 붕괴 보수 — ⚡ 오늘 빌드(P-069) 전 포함 목표

3기기(아이폰14P·XS·S22) 재현: **원거리 촬영으로 줄 간격이 조밀하면** 마커들이 좌우 열 번갈아 **정확히 일정 간격 사다리**로 쌓임(스샷 실측 — 스케일 오차 아님, 간격 균일이 지문). 커맨드 센터 가설(확신 80%): ① `X_CLUSTER=150pt`가 이 구도에서 좌우 열(간격 ~145pt)을 한 클러스터로 병합 ② 촘촘한 줄 간격(<PILL_H 34)에서 스태거가 연쇄 발동 → 전 마커 34pt 계단. 근접 촬영이 정상인 이유·기기 무관성 모두 설명됨.

### 할 일

0. **가설 검증 먼저**: `positions` 순수 함수에 조밀 2열 좌표 분포(줄 간격 ~0.035, 두 열 x 간격 ~0.37) 유닛 재현 — 사다리 발생 확인 후 착수, 아니면 원인 재보고
1. **클러스터 판정 실폭 기반**: 고정 150pt → pill 실제 렌더 폭(텍스트 길이 기반) 겹침 판정으로 — 좌우 열 분리가 목적
2. **스태거 누적 상한**: 원 앵커에서 최대 1단(PILL_H)까지만 — 그 이상 겹침은 허용(연쇄 사다리 금지). 밀집도가 높으면 겹침이 남는 건 수용(핀치 줌으로 확대 가능)
3. 유닛: 조밀 2열 = 사다리 미발생 잠금 + 기존 근접 케이스 무회귀
4. **P-069 빌드 전 머지 목표**(임베드) — 늦으면 OTA. 실기기(문제의 노트북 화면 원거리 촬영) 확인 후 발행

### DoD

- [ ] 조밀 2열 유닛 사다리 0 · 근접 무회귀 · 실기기 원거리 재현 해소 스샷 · tsc 0 · jest

완료 시 상태 ✅+커밋 해시, 보고는 REPORTS.md 최상단 [P-070].

## [P-069] ✅ d817c73·f1c245c (iOS 2328c807 · 안드 29328c51) KB-194 QA용 빌드 — iOS 빌드8 + 안드 빌드3, **env=dev** (오늘 실기기 QA용)

예진 확정(7/24): 오늘 = QA(네이티브 변경분 포함), 스토어 업로드 = 내일. prod 미시딩이므로 **오늘 빌드는 dev.kbap.site를 임베드** — BE 주소는 JS env라 네이티브 지문 무관, 오늘의 네이티브 QA는 내일 제출 빌드에도 그대로 유효.

### 할 일

1. `eas.json` preview·production env → `EXPO_PUBLIC_BE_BASE=https://dev.kbap.site` (**임시** — 제출 빌드에서 prod로 교체 예정, 주석 명시)
2. **iOS 빌드8**: eas build + 테플 제출 — P-054 스플래시·supportedLocales 등 네이티브 전부 탑승, 재베이스라인(스왑 절차 소멸)
3. **안드 빌드3**: 공기계 설치용 APK(내부 배포 링크 보고) — AAB는 내일 제출 빌드에서
4. 보고: 빌드 ID·runtime 지문·임베드 커밋(0950e5a 이후 최신 main) · tsc·jest 그린 확인

### DoD

- [ ] 테플 빌드8 설치 가능 · 안드 빌드3 APK 링크 · 임베드=최신 main+dev 배선 실증

완료 시 상태 ✅+커밋 해시·빌드 ID, 보고는 REPORTS.md 최상단 [P-069].

### 내일(7/25) 제출 라운드 예고 — 별도 P로 발주 예정

종한 prod 시딩+배포 신호 → P-067(prod 배선) → **제출 빌드(iOS 빌드9·안드 빌드4 AAB, env=prod)** → 스토어 업로드. 네이티브 동일 — 내일은 스모크만.
...
## [P-068] 🔄 0950e5a KB-233 리스트 뷰 하단 버튼 — A안(배경 페이드 + 바닥 여백) 확정

예진 확정(7/23, 목업 A안): 리스트 뷰에서 카드가 플로팅 버튼과 겹쳐 안 보이는 문제.

### 할 일

1. **배경 페이드**: 리스트 뷰 하단에 **배경색(리스트 다크 배경과 동일 색) 그라데이션**(투명→불투명 ~96%, 높이 ~150px) — 버튼 뒤 카드가 자연스럽게 잠기게. P-066 사진 뷰 그라데이션과 같은 레이어 순서 규칙(버튼 뒤·리스트 위, 안드 elevation 포함)
2. **바닥 여백**: 리스트 contentContainer paddingBottom = 버튼 영역+여유(~140px) — **끝까지 스크롤하면 마지막 카드가 버튼 위로 완전히 올라옴**(가려지는 카드 0)
3. 배너·카드·버튼 로직 무변. 테스트: paddingBottom 잠금 1건
4. 실기기 확인 후 preview OTA (P-066과 같은 OTA 합류 가능)

### DoD

- [ ] 끝 스크롤 시 마지막 카드 완전 가시 · 버튼 아래 카드 페이드 · tsc 0 · jest · OTA

완료 시 상태 ✅+커밋 해시, 보고는 REPORTS.md 최상단 [P-068].

## [P-066] 🔄 90e9948 KB-233 [반려 보강] 결과 하단 그라데이션 — 실기기 미표시·파파고 수준으로

예진 실기기: 사진 뷰 하단 그라데이션이 **안 보임**(코드엔 존재 — resultShade 170px·0→0.55). 파파고 캡처 기준으로 보강.

### [커맨드 센터 추가 7/23 19:35 — 같은 건으로 보수]

Metro 실기(예진 아이폰, 위험도 뷰) 스샷 증상 2건 추가:
- **버튼 행이 화면 최하단에 붙어 밀림** — safe-area(insets.bottom) 미반영 의심(라벨이 바닥에 닿음)
- **마커 필이 버튼 행 높이·아래까지 렌더**되어 버튼과 겹침 — 그라데이션 부재와 겹쳐 가독 붕괴.
  마커는 사진 위 자유 배치가 맞지만(파파고 동일), 버튼 행이 마커 **위에**(z) + 그라데이션 뒤받침 필수
...
1. **스캔라인 왕복**: `withRepeat(..., -1, false)` → **`true`(reverse)** — 위↔아래 핑퐁(scan.tsx 스캐닝 오버레이). 리셋 점프 소멸
2. **결과 화면 파파고식(위험도·원본 뷰)**: 하단 **다크 시트 컨테이너 삭제** — 사진을 화면 최하단까지 풀블리드 + 사진 하단에 **옅은 그라데이션 섀도만**(버튼 가독용, 살짝). **캡션(`scan.resultCaption`)·위험도 범례 칩 3개 삭제**(방금 만든 i18n 키 ×10 정리 포함) → **원형 버튼 4개만 플로팅**(리스트/위험도/원본/다시찍기 · 활성=주황 · 라벨 유지 · safe-area 하단 여백)
3. **원본 피크**: 위험도 뷰에서 마커 없는 영역을 **꾹 누르면 오버레이(마커·버튼) 페이드아웃 → 원본 사진만**, 떼면 복귀. 마커 탭(상세 진입)과 간섭 없게
4. **핀치 줌+팬**(위험도·원본 뷰): gesture-handler(기설치) Pinch+Pan — **이미지+마커를 같은 transform 컨테이너**에 (확대해도 마커 정위치 추종). 스케일 1x~4x 클램프·경계 팬 클램프·**더블탭 = 줌 토글/리셋**. 줌 상태에서도 마커 탭 동작
5. 테스트: reverse 잠금 · 캡션/범례 부재+버튼 4 · 피크 토글 · transform 클램프 유닛

### DoD

- [ ] 스캔라인 왕복 · 검정 바탕 0(사진 풀블리드+하단 섀도) · 버튼만 플로팅 · 꾹=원본/뗌=오버레이 · 핀치 줌 시 마커 정위치 · tsc 0 · jest · OTA

완료 시 상태 ✅+커밋 해시, 보고는 REPORTS.md 최상단 [P-064].
...
0. **[반려 보수] 셔터 가드 레이스**: 실기기 재현 — 연타가 여전히 다중 촬영(예진 ⚠️). 원인 = `capturing` **state** 가드는 리렌더 전 탭을 못 막음(비동기 스테일 클로저). **`useRef` 동기 가드**(진입 즉시 `ref.current` 검사·세트)로 교체, state는 시각적 disable 전용으로 병행. capture·pickFromGallery 동일. **연타 레이스 테스트 추가**(연속 2회 호출 → takePictureAsync 1회 잠금)
1. **Run sample scan 제거**: 두 곳(카메라 화면·권한 거부 화면) 모두 — SAMPLE_DISHES·`scan.sample` i18n 키(×10) 정리. 권한 거부 화면은 갤러리 버튼 안내가 남도록
2. **스캔 로딩 = D2 스캐닝 오버레이**: 현행 검은 화면+스피너 폐기 → **촬영/선택한 사진이 그대로 배경**, 그 위에 ① 흰 코너 브래킷 4개(뷰파인더 프레임) ② **주황(#E2580C 계열) 수평 스캔라인이 위→아래 반복 스윕**(부드러운 글로우 그라데이션 트레일, reanimated 무한 루프) ③ 하단 캡션 = 기존 `scan.reading` i18n 재사용(신규 키 없음) + 소형 스피너 ④ 좌상단 X 유지(취소). 갤러리 선택 경로도 동일
3. **결과 하단 바 = D3 스타일**: 다크 시트(상단 라운드+드래그 핸들) 재구성 —
   - 상단 캡션 "Tap a dish for your risk & ingredients"(신규 i18n 키 ×10) + **위험도 범례 칩 3개**(Danger·Caution·Safe — 기존 위험도 라벨 i18n 재사용, 색+SVG 아이콘 병행: 색맹 축·헌법 III)
   - **원형 버튼 4개(라벨 하단)**: 리스트 / 메뉴판위(위험도 오버레이) / 원본 / **다시찍기** — 기존 세그먼트·버튼 대체. **사진저장 제외**(예진 확정). 라벨은 기존 i18n 키(목록/위험도/원본/다시찍기) 재사용
   - **활성 뷰 = 주황 원 배경**(D3의 Ask owner 스타일), 비활성·다시찍기 = 어두운 원. 아이콘 전부 SVG(기호 텍스트 렌더 금지)
4. 테스트: 셔터 레이스(0번) · 하단 바 4버튼/활성 뷰 잠금 · 샘플 버튼 부재

### DoD

- [ ] 연타=1촬영(ref 가드) · 샘플 버튼 0곳 · 로딩=사진+스캔라인 오버레이 · 하단 바=캡션+범례+4버튼(활성 주황) · tsc 0 · jest · preview OTA

완료 시 상태 ✅+커밋 해시, 보고는 REPORTS.md 최상단 [P-062].
