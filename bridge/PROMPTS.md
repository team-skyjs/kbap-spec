...
## [P-069] ⬜ KB-194·KB-82 최종 빌드 — iOS 빌드8 + 안드 빌드3 (7/24 제출용) — ⛔ 게이트: P-066·P-067·P-068 완료+검토 후

예진 지시(7/24): 테플·안드 재배포. **검증한 번들 = 제출 번들** — 아래 게이트가 전부 닫혀야 착수.

### 선행 게이트 (전부 ✅ 확인 후 착수)

- [ ] P-066·P-068 완료 + 커맨드 센터 검토 통과 (스캔 화면 완성 상태로 임베드)
- [ ] P-067 완료 (빌드 대상 = **prod.kbap.site**, meogo 참조 0) — 종한 prod 시딩+배포 신호 필수 (7/24 아침 실측: prod DB 아직 0건)

### 할 일

1. **iOS 빌드8**: eas build → TestFlight 제출. P-054 이후 네이티브 전부 탑승(안드 스플래시 에셋·supportedLocales·app.json 현행) — **재베이스라인으로 OTA 스왑 절차 소멸** 명시
2. **안드 빌드3**: 스토어 제출용 **production AAB** + 공기계 검증용 APK 병행(프로파일 구성은 eas.json 기준 보고에 명시)
3. 빌드 완료 시 보고: 빌드 ID·runtime 지문·임베드 커밋 해시 — 이후 Q-20 빌드 게이트 확인(스플래시·OS 언어 노출·스캔 언어·A90 폴백)은 예진
4. ⚠️ 빌드 전 최종 확인: `.env`(dev)는 빌드에 안 스며드는지(eas.json env가 prod 명시), tsc 0 · jest 그린

### DoD

- [ ] 테플 빌드8 설치 가능 · 안드 빌드3 AAB+APK 산출 · 임베드 번들=최신 main · 빌드 대상=prod 실증
