...
## [P-012] ⬜ KB-179 음식 상세 가격 표시 (착수 게이트: BE price 필드 배포 확인)

QA Q-01 피드백(7/20): 스캔 리스트엔 가격이 뜨는데 상세엔 없음. 실측 결과 **FoodDetailResponse에 price 미배포**(스펙·라이브 모두) — BE 배포 후 착수. **착수 시 Swagger 실측: price 없으면 착수 보류 보고만.**

### 할 일 (price 배포 확인 후)

1. foodAdapter 상세 매핑에 price 추가 — 스캔과 동일 규칙(Number.isFinite 방어, null=미표시)
2. 상세 화면에 가격 표시 — 기존 레이아웃(이름·스파이스·위험도 구역) 깨지지 않는 위치, ₩ 포맷만(환율·추정 금지)
3. 테스트: 가격 정상/null 매핑 + 렌더 유무

### DoD

- [ ] 가격 있는 음식 ₩ 표시 / 없는 음식 가격 영역 미노출 · tsc 0 · jest 통과

완료 시 상태 ✅+커밋 해시, 보고는 REPORTS.md 최상단 [P-012].

## [P-011] ⬜ KB-178 기피 재료 선택 UI 개선 — 요약 가로 스크롤 1줄 + CTA 하단 고정 (B안 확정)
