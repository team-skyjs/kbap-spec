...
## [P-056] ⬜ KB-176 안드 스캔 오버레이 — 마커 위치 어긋남(일정 하향 밀림) + OCR 인식률 조사

실기(예진 7/22, 안드 빌드2 공기계): 같은 스캔의 목록엔 6개 정상인데 **위험도(오버레이) 뷰**에서 ① 마커가 2개(김치찌개·된장찌개)뿐 ② 그 2개도 **실제 위치보다 일정하게 아래로 밀림**(김치찌개 마커가 부대찌개 옆, 된장찌개 마커가 닭볶음탕 옆).

### 판정 가이드 (커맨드 센터 분석)

- **마커 2개**: photoOnly(서버 추출, 좌표 없음)는 마커 미표시가 스펙 — 즉 **안드 OCR이 6개 중 2개만 dish로 세그먼트**한 것이 실체. iOS 동일 메뉴판 대비 인식률 차이 조사 필요(치명 아님 — 목록은 완전).
- **위치 밀림 = 진짜 버그**: 일정 오프셋 하향 — **안드 ML Kit 좌표계 불일치** 유력: (a) EXIF 회전 미보정 프레임 좌표 (b) 보고 dims(point?) vs 실측 픽셀 불일치(iOS에서 KB-141 때 잡은 그 계열의 안드판) (c) **P-025 크롭 전 이미지 기준 좌표를 크롭 후 이미지에 매핑**했을 가능성 — 크롭 오프셋만큼 어긋남과 정합적인지 우선 확인.

### 할 일

1. 안드 실기/에뮬에서 `[ocr] dims`·`[scan] segmented` 로그로 **좌표 정규화 기준 치수 vs 실제 표시 이미지 치수** 대조 — 원인 (a)/(b)/(c) 확정해 보고에 명시
2. 확정 원인 수정 (크롭 순서 문제면 OCR을 크롭 후 이미지에 수행/좌표 보정, dims 문제면 안드 분기 실측 픽셀 사용)
3. 인식률(2/6)은 별도 항목으로 조사 결과만 보고(개선은 후속 판단) — 좌표 수정과 분리
4. 테스트: 좌표 변환 함수에 안드 케이스 잠금. iOS 무회귀
5. JS-only면 preview OTA / 네이티브 관여 시 명시

### DoD

- [ ] 안드 오버레이 마커가 해당 메뉴 옆에 정위치 · 원인 보고 · iOS 무회귀 · tsc 0 · jest
