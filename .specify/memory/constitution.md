...
Version change: 2.0.0 → 2.1.0  (2026-07-23 원칙 III 적용 범위 명확화 — 예진 확정)
  - 회피 재료 **미설정** 사용자에 대한 위험도 표시: 종전 FE 강등 규칙(safe→주의)을 폐지하고
    BE 판정 그대로 노출한다. 근거: 전 항목 '주의' 표시는 정보량 0으로 둘러보기 UX를 파괴.
    안전장치 = 첫 스캔 '기피 설정' 유도 배너 유지 + 안전 고지 페이지에 "미설정 시 일반
    정보이며 개인 맞춤 판정 아님" 명시. 원칙 III의 '근거 없는 안전 단정 금지'는 **개인화
    판정 맥락**(기피 설정 사용자)에 적용되는 것으로 명확화.
...
- 불확실성은 사용자에게 위험을 낮추는 방향으로 처리한다(MUST). 정보가 불완전하면 '안전'으로
  ...
  유도한다.
- 위 '안전 단정 금지'는 **기피 재료를 설정한 사용자의 개인화 판정**에 적용된다. 회피 미설정
  사용자에게는 BE 판정을 그대로 표시하되(2026-07-23, v2.1.0), 이것이 개인 맞춤 판정이
  아님을 첫 스캔 유도 배너와 안전 고지로 알린다.
...
**Version**: 2.1.0 | **Ratified**: 2026-06-15 | **Last Amended**: 2026-07-23
